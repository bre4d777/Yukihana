...
class CV2Command extends Command {
  ...
  buildMainContainer(username, client) {
    ...
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`# ${client.emoji.rocket} Components v2 Showcase\n*Demonstrating every single feature available*\n\nWelcome **${username}**!`)
    );
    ...
    container.addSectionComponents(
      new SectionBuilder()
        .addTextDisplayComponents(
          new TextDisplayBuilder().setContent(`## ${client.emoji.chart} Live Statistics`),
    ...
    container.addSectionComponents(
      new SectionBuilder()
        .addTextDisplayComponents(
          new TextDisplayBuilder().setContent(`## ${client.emoji.star} Key Features`),
          new TextDisplayBuilder().setContent(this.demoData.features.map(f => `${client.emoji.success} ${f}`).join('\n'))
        )
    ...
    container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`## ${client.emoji.gift} Featured Products`));
  ...
  buildInteractionLogContainer(userId, client) {
    ...
    container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`# ${client.emoji.chart} Interaction Log\n*Your activity during this demo session*`));
    container.addSeparatorComponents(this._createSeparator(SeparatorSpacingSize.Small, true));
  ...
  buildSpoilerContainer(client) {
      return new ContainerBuilder().setAccentColor(0x747F8D).setSpoiler(true)
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`# ${client.emoji.eye} Spoiler Mode Demo\n*This entire container is now hidden behind a spoiler*`))
  ...
  buildErrorContainer(error, client) {
    return new ContainerBuilder().setAccentColor(0xED4245)
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`## ${client.emoji.error} An Error Occurred\n${error}`));
  }
  ...
  async execute({ client,message,args }) {
    ...
    collector.on('collect', async interaction => {
      ...
      try {
        ...
        if (customId === 'nav_back_to_main') {
            ...
            responseContainer = this.buildMainContainer(interaction.user.username, client);
        }
            ...
            if (action === 'refresh_stats') {
                ...
                responseContainer = this.buildProductDetailContainer(productId, client);
            ...
            if (action === 'purchase') {
                ...
                await interaction.followUp({ content: `${client.emoji.star} Added **${product.name}** to your favorites! (Demo)`, ephemeral: true });
            }
            return; 
        }
            ...
            if(action === 'clear') {
                ...
                responseContainer = this.buildInteractionLogContainer(userId, client);
            }
        }
    ...
    collector.on('end', async () => {
      const expiredContainer = new ContainerBuilder().setAccentColor(0x747F8D)
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`# ${client.emoji.rocket} Components v2 Showcase\n*This demo has expired or was closed.*\n\nRun the command again to start a new demonstration.`));
  ...
  async slashExecute({ interaction, client }) {
    ...
    await this.execute({ client, message: interaction, args: mode ? [mode] : [] });
  }
